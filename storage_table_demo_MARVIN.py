#THIS IS A COMMENT ADDED BY MARVIN
...
resourcegroup_name = 'mtd'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'mtd'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'eastus'
...
#Cars are characterized by make, model, year, color and price.
...
car.price = 35000
table_service.insert_entity('itemstable', car)
print('Created entry for lexus is250...')
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'cardealership'", select='make,model,price')
for item in items:
    print('Make: ' + item.make)
    print('Model: ' + str(item.model) + '\n')
    print('Price: ' + str(item.price) + '\n')

items = table_service.query_entities('itemstable', filter="PartitionKey eq 'coffeeshop'", select='brand,cost')
for item in items:
